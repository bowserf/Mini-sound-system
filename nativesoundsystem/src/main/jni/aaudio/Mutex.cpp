...
Mutex::~Mutex() {
    ...
    if (res != 0) {
        LOGE ("Mutex::~Mutex: fail destroy; error=0x%X", res);
    }
}
...
void Mutex::lock() {
    ...
    if (res != 0) {
        LOGE ("Mutex::lock: fail lock; error=0x%X", res);
    }
}
...
void Mutex::unlock() {
    ...
    if (res != 0) {
        LOGE ("Mutex::unlock: fail unlock; error=0x%X", res);
    }
}
...
bool Mutex::tryLock() {
    ...
    if ((res != 0)) {
        LOGE ("Mutex::tryLock: error=0x%X", res);
    }
    return res == 0;
}
